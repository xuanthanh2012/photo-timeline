  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="relative max-w-4xl w-full max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="bg-secondary/80 backdrop-blur-sm p-4 rounded-lg text-center flex-shrink-0 flex justify-between items-center">
          <div>
            <p className="text-text-main font-semibold text-left">{photo.caption || 'No caption'}</p>
            <p className="text-text-secondary text-sm text-left">{formattedDate}</p>
          </div>
          <button
            onClick={handleDelete}
            className="text-text-secondary hover:text-red-500 transition-colors p-2 rounded-full"
